// Seed script: creates a test church + 3 users (admin, musico, multimedia)
...
const IglesiaSchema = new mongoose.Schema(
  { nombre: String, slug: String, plan: { type: String, default: 'free' }, estadoSuscripcion: { type: String, default: 'activa' } },
  { timestamps: true },
)
...
const USERS = [
  ...
  { nombre: 'Músico Demo',     email: 'musico@demo.com',     password: 'Musico1234',     rol: 'musico'     },
...
// Create church
const iglesia = await Iglesia.create({ nombre: 'Iglesia Demo', slug: IGLESIA_SLUG })
console.log(`✓ Iglesia creada: "${iglesia.nombre}" (slug: ${iglesia.slug})`)

// Create users
for (const u of USERS) {
  const passwordHash = await hash(u.password, 12)
  await Usuario.create({ iglesiaId: iglesia._id, nombre: u.nombre, email: u.email, passwordHash, rol: u.rol, activo: true })
